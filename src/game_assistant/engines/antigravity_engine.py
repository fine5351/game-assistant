...
class AntigravityCliEngine:
    ...
    def decompose_user_demand(
        ...
        prompt = (
            ...
            "【系統實體操作與自律生長能力認知】：\n"
            "本助理系統已配備完整的『自律器官生長器 (OrganSynthesizer)』與『鍵鼠實體致動器 (ScreenActuator)』，能夠即時自律生長出專屬的 Python 操作 Script 宏手並接管操作！\n"
            "嚴禁宣稱『物理終端（手）缺失』或『只有玩家有手』。\n"
            "當玩家要求操作接管、自動戰鬥、執行連招或提問『長出手』時，請提供清晰結構化的即時戰術指示 (Directive)，格式規範：\n"
            "1. 核心目標 (例如：自律生長操作手腳本、接管執行連招、核爆終結)\n"
            "2. 推薦輸入序列 (明確列出鍵盤與滑鼠序列，如：切 3 號位 -> E -> 普攻 2 次 -> 切 4 號位 -> E -> Q -> 切 1 號位 -> Q)\n"
            "3. 警戒條件 (例如：遇紅光閃避無敵幀頂傷害、技能冷卻與能量警報)\n"
            "請以繁體中文回答，條點清晰，適合手部器官 (DynamicActionScriptHand) 與 Jev 反射神經直接解析執行。"
        )
